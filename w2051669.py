...
def credit_input(prompt):
    while True:
        try:
            ...
            if credits not in [0, 20, 40, 60, 80, 100,
                               120]:  # if user enterd a value not in this range it will print "Out of range!'
                print("Out of range!")
                continue
            return credits
        except ValueError:
...
while True:
    ...
    if total_credits != 120:
        print(
            "Total incorrect!")  # If the total is not equals to 120 it will print "Total incorrect!" and it will the user to add the credits again. And if the total is equals to 120, it will give a choice to the user.
    else:
        if pass_credits == 120:
            print("Progress\n")
        ...
        result_data.append([pass_credits, defer_credits,
                            fail_credits])  # Takes these credit values and adds them as a list to result_data.
        ...
        if choice == "q":
            ...
            total_count = (
                        Progress + Progress_module_trailer + Do_not_progress_module_retriever + Exclude)  # finds the total count for later.
            max_count = max(Progress, Progress_module_trailer, Do_not_progress_module_retriever,
                            Exclude)  # finds the Progression type wich has the most counts.
            ...
            def display_histogram():
                try:
                    ...
                    # Bar 1,
                    bar1_top_text = Text(Point(140, bar1_height - 10),
                                         f"{Progress}")  # Prints the progress count on the bar.
                    ...
                    bar1 = Rectangle(Point(80, 400), Point(200,
                                                           bar1_height))  # Draws the bar.Height is decided by the formula(bar1_height).
                    ...
                    # Bar 2,
                    bar2_top_text = Text(Point(280, bar2_height - 10),
                                         f"{Progress_module_trailer}")  # Prints the Progress_module_trailer count on the bar.
                    bar2_top_text.setStyle("bold")
                    bar2_top_text.setTextColor("grey")
                    bar2_top_text.setSize(16)
                    bar2_top_text.draw(win)

                    bar2 = Rectangle(Point(220, 400), Point(340,
                                                            bar2_height))  # Draws the bar.Height is decided by the formula(bar2_height).
                    ...
                    # Bar 3,
                    bar3_top_text = Text(Point(420, bar3_height - 10),
                                         f"{Do_not_progress_module_retriever}")  # Prints the Do_not_progress_module_retriever count on the bar.
                    bar3_top_text.setStyle("bold")
                    bar3_top_text.setTextColor("grey")
                    bar3_top_text.setSize(16)
                    bar3_top_text.draw(win)

                    bar3 = Rectangle(Point(360, 400), Point(480,
                                                            bar3_height))  # Draws the bar.Height is decided by the formula(bar3_height).
                    ...
                    # Bar 4,
                    bar4_top_text = Text(Point(560, bar4_height - 10),
                                         f"{Exclude}")  # Prints the Exclude count on the bar.
                    bar4_top_text.setStyle("bold")
                    bar4_top_text.setTextColor("grey")
                    bar4_top_text.setSize(16)
                    bar4_top_text.draw(win)

                    bar4 = Rectangle(Point(500, 400), Point(620,
                                                            bar4_height))  # Draws the bar.Height is decided by the formula(bar4_height).
                    ...
                    # Bottom Text,
                    bottom_text = Text(Point(180, 450),
                                       f"{total_count} out comes in total.")  # Prints the totla count of progression in the bottom.
            ...
            progression_order = ["Progress", "Progress (module trailer)", "Do not Progress (module retriever)",
                                 "Exclude"]
            # 'progression_order' list is used to define progression in specific order.

            f = open("Part 3(w2051669).txt", 'w')  # Opens a File named Part 3(w2051669).
            print("Part 3:", file=f)
            print("\nPart 2:")
